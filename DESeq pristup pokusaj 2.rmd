...
```{r}
codonsInSamples2
getID(codonsInSamples2)

uzorci <- data.table(sample = samples, KO = getKO(codonsInSamples2), codonCounts(codonsInSamples2))
```

```{r}
subset(codonsInSamples2, getKO(codonsInSamples2) == "K11751")
nj <- subset(codonsInSamples2, substr(getID(codonsInSamples2), 1, 3) == "HD1")

unique(samples)
```

```{r}
tiny <- codonsInSamples2[1:50000]
uzorci <- data.table(sample = str_extract(getID(tiny), "[HL]D\\d+"), KO = getKO(tiny), codonCounts(tiny))


uzorci[, melp := MELP(tiny, subsets = sample), by = sample]
uzorci[, melp := MELP(tiny, subsets = (list(selected = str_extract(getID(tiny), "[HL]D\\d+") == sample)), ribosomal = TRUE), by = sample]

#E sad sredi ovaj dio da radi
```
