...
Ovo slozi s data table:
-verzija s dplyr pipeline-om:
```{r}
clustering <- function(N){
  ...
  missclass <- data.table(cluster = 1:N, mc_rate = t(mc_rate))
  missclass$mc_rate
}
...
```{r}
#daj prokuzi ovo
kclustering <- kmeans(distances, 3)
classy <- data.table(condition = substr(names(kclustering$cluster), 1, 1), cluster = kclustering$cluster)
missclass <- data.table(cluster = 1:3)

missclass[ , mc_rate := min()]
...
consensus_clst <- function(N){
  clusters <- matrix(replicate(1000, klasteri(N)), ncol = 1000)
  clusters <- data.table(cbind(sample = tablica$`Sample ID`, clusters))
  clusters[, final_cluster := which.max(table(as.matrix(.SD))), .SDcols = V2:V1001, by = sample] #samo trebas funkciju s ovom linijom
  return(clusters$final_cluster)
}
```
...
clustering <- function(N){
  ...
  missclass <- data.table(cluster = 1:N, mc_rate = t(mc_rate))
  #return(missclas)
  data.table(mc_rate = list(missclass), cluster = list(kclustering$cluster))
  #return(data.table(missclass, kclustering$cluster))
}




#ponocno napraci da vraca dt s missclass i dt s konsenzus klasterima:
#opet ce biti isto kada ce se zasebno pozivati 3 clustering funkcije
mc_table <- function(N){
  missclass <- data.table(cluster = 1:N, 
                          mc_rate = apply(replicate(1000, unlist(clustering(N)$mc_rate[[1]][, 2])), 1, sum) / 1000,
                          st_dev = apply(replicate(1000, unlist(clustering(N)$mc_rate[[1]][, 2])), 1, sd))
  consensus <- unique(apply(replicate(1000, unlist(clustering(N)$cluster)), 1, consensus_clst))
  return(data.table(missclass = list(missclass), consensus = list(consensus)))
}
...
#znaci poslozi ovo sve u matricu s replicate i onda od tog tek racunaj konsenzus klaster
consensus_clst <- function(clusters){
  consensus <- data.table(clusters)
  consensus[, final_cluster := which.max(table(as.matrix(.SD))), by = 1:nrow(consensus)] #samo trebas funkciju s ovom linijom
  return(consensus$final_cluster)
}

consensus_clst(unlist(clustering(3)$cluster))
which.max(table(as.matrix(unlist(nj$cluster))))
