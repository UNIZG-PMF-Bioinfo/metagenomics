...
```{r}
#cl_count <- data.frame(matrix(rep(0, 60*60), ncol = 60))
#colnames(cl_count) <- tablica$`Sample ID`
#rownames(cl_count) <- tablica$`Sample ID`


#ovo bi trebalo dati konsenzus matricu, sad jos prokuzi kako ih na temelju toga podijeliti u klastere
proba <- ConsensusClusterPlus(distances, maxK = 10, reps = 1000, pFeature = 1, pItem = 0.8, distance = "euclidean", clusterAlg = "km")
proba[[2]][["consensusMatrix"]]
proba[[3]][["consensusMatrix"]]
