...
```{r}
imena <- rownames(res[res$padj < 0.01 & !is.na(res$padj),])
```


```{r}
anot <- function(KO){
  description <- keggGet(KO)
  return(description[[1]]$DEFINITION)
}
```

```{r}
KOs <- data.table(KO = rownames(res[res$padj < 0.05 & !is.na(res$padj),]))
KOs[, description := lapply(KO, anot)]
KOs[, description := anot(KO), by = 1:nrow(KOs)]
```
