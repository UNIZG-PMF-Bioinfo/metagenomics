...
```{r}
KOs <- rownames(res[res$padj < 0.1 & !is.na(res$padj),])[1:5]

ensembl <- useMart("ensembl")
ensembl <-  useDataset("hsapiens_gene_ensembl", mart = ensembl)

proba <- getBM(attributes = "pdb",
               filters = "uniprot_gn",
               values = KOs$protein[1],
               mart = ensembl)
...
dataset <- DESeqDataSetFromMatrix(countData = dt_filtered, colData = coldata2, design = ~ condition)
#head(assay(dataset))
```
...
PCA_1st_filtering <- ggbiplot(PCA, ellipse = TRUE, var.axes = FALSE, groups = factor(substr(rownames(rlog_values), 1, 1))) + theme_classic()
```
...
dataset <- DESeqDataSetFromMatrix(countData = dt_filtered2, colData = coldata3, design = ~ condition)
```
...
PCA_2nd_filtering <- ggbiplot(PCA, ellipse = TRUE, var.axes = FALSE, groups = factor(substr(rownames(rlog_values), 1, 1))) + theme_classic()
```
...
```{r}
KOs <- data.table(KO = rownames(res[res$padj < 0.1 & !is.na(res$padj),]))
KOs <- KOs[-c(74, 162)] #izbaceni su K03081 i K08345 jer ih nema u KEGG dt
KOs[, name := (keggGet(KO)[[1]])$NAME, by = 1:nrow(KOs)]
KOs[, definition := (keggGet(KO)[[1]])$DEFINITION, by = 1:nrow(KOs)]
